                ...
                <div className="bg-white dark:bg-gradient-to-br dark:from-[#1e1e1e] dark:to-[#2b184a] secret:bg-none secret:bg-black border-4 border-black dark:border-[#a855f7] secret:border-[#1cf85d] p-6 shadow-[8px_8px_0px_#06b6d4] dark:shadow-md flex flex-col secret:rounded-none h-fit">
                    ...
                    {generatedRef ? (
                        <div className="flex flex-col h-full justify-between">
                            ...
                            >
                                {isCopied ? (
                                    <><Check className="w-6 h-6 mr-2 font-bold" /> {t('ref.copied')}</>
                                ) : (
                                    <><Copy className="w-6 h-6 mr-2 font-bold" /> {t('ref.copy')}</>
                                )}
                            </button>
                        </div>
                    ) : (
                        <div className="flex-1 flex flex-col items-center justify-center text-black dark:text-gray-500 secret:text-[#1cf85d]/40 min-h-[200px]">
                            <BookMarked className="w-12 h-12 mb-2 opacity-50" />
                            <p className="text-center font-bold text-sm secret:font-mono uppercase">&gt; {t('ref.empty')}</p>
                        </div>
                    )}
                </div>
            </div>
