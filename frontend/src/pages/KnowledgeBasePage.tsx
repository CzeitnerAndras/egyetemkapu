...
export default function KnowledgeBasePage() {
    ...
    return (
        <main className="w-full max-w-7xl mx-auto mt-6 pb-12 px-4 relative z-20">
            ...
            {/* --- Dokumentumok Grid --- */}
            {loading ? (
                <p className="text-center text-gray-500 secret:text-[#1cf85d] secret:font-mono">Fájlok betöltése...</p>
            ) : documents.length === 0 ? (
                <p className="text-center text-gray-500 dark:text-gray-400 secret:text-[#1cf85d]/70 font-medium text-lg secret:font-mono uppercase">&gt; Nincsenek fájlok ebben a kategóriában.</p>
            ) : (
                    ...
                    {documents.map(doc => (
                        <div key={doc.id} className="border-4 border-[#800000] dark:border-[#a855f7] secret:border-[#1cf85d] bg-gradient-to-br from-[#fdfbf7] to-[#f4ebe1] dark:from-[#1e1e1e] dark:to-[#2b184a] secret:bg-none secret:bg-black p-5 flex flex-col shadow-md hover:shadow-lg transition-shadow secret:rounded-none group">
                            <div className="flex justify-between items-start border-b-2 border-gray-300 dark:border-gray-700 secret:border-[#1cf85d] pb-3 mb-3">
                                <div>
                                    <span className="text-xs font-bold bg-[#800000] dark:bg-[#c084fc] secret:bg-[#1cf85d] text-white secret:text-black px-2 py-1 rounded-sm secret:rounded-none uppercase tracking-wider">{doc.category}</span>
                            ...
                            <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 secret:text-[#1cf85d]/60 mb-4 secret:font-mono">
                                <span>Feltöltötte: <span className="font-bold">{doc.uploader.username}</span></span>
                            ...
                            >
                                <Download className="w-4 h-4 mr-2" /> Letöltés
                            </button>
                        </div>
                    ))}
                </div>
            ...
            {isUploadModalOpen && (
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                    <div className="bg-white dark:bg-[#1e1e1e] secret:bg-black border-4 border-[#800000] dark:border-[#a855f7] secret:border-[#1cf85d] w-full max-w-lg p-6 relative shadow-[0_0_50px_rgba(0,0,0,0.5)] secret:shadow-[0_0_30px_rgba(28,248,93,0.3)] secret:rounded-none">
                        ...
                        <h2 className="text-2xl font-bold text-[#800000] dark:text-[#c084fc] secret:text-[#1cf85d] mb-4 border-b-2 border-[#800000] dark:border-[#a855f7] secret:border-[#1cf85d] pb-2 secret:font-mono uppercase">Új anyag beküldése</h2>
                        ...
                        <form onSubmit={handleUpload} className="space-y-4">
                            ...
                            <div className="flex flex-col">
                                <label className="text-sm font-bold text-[#800000] dark:text-[#c084fc] secret:text-[#1cf85d] mb-1 secret:font-mono uppercase">Leírás</label>
                            ...
                            <div className="flex flex-col">
                                <label className="text-sm font-bold text-[#800000] dark:text-[#c084fc] secret:text-[#1cf85d] mb-1 secret:font-mono uppercase">Kategória</label>
                            ...
                            <div className="flex flex-col">
                                <label className="text-sm font-bold text-[#800000] dark:text-[#c084fc] secret:text-[#1cf85d] mb-1 secret:font-mono uppercase">Fájl (PDF, Word, stb.)</label>
                            ...
                            <button type="submit" className="w-full bg-[#800000] dark:bg-[#a855f7] secret:bg-transparent text-white secret:text-[#1cf85d] font-bold py-3 mt-4 border-2 border-black dark:border-transparent secret:border-[#1cf85d] hover:bg-[#b91c1c] secret:hover:bg-[#1cf85d] secret:hover:text-black transition-colors cursor-pointer secret:font-mono uppercase">
                                Feltöltés és Küldés jóváhagyásra
                            </button>
                        </form>
                    </div>
                </div>
